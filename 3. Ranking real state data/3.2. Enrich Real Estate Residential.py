...
# MAGIC 3.2. Feature Engineer Real Estate Data - Enrich Real Estate Residential
# MAGIC 
# MAGIC ![](/files/images/plan10.png)
...
# DBTITLE 1,Grab Geofenced Real Estate Data into Business and Residential
#load geofenced data
...
display(geofenced_residential)

# COMMAND ----------

# DBTITLE 1,Save Separated Real Estate Geofenced Residential Data to Delta Lake
#spark.sql("DROP TABLE IF EXISTS workshop.geofenced_real_estate_residential")
#geofenced_residential.write.format("delta").saveAsTable("workshop.geofenced_real_estate_residential")
#Another option: saving as temp view instead 
#geofenced_residential.createOrReplaceTempView("temp_geofenced_real_state_residential")

# COMMAND ----------

# MAGIC %md
# MAGIC ### 3.2.1 Residential Address Enrichment 
# MAGIC 
# MAGIC * We are following same process as we did for Training Data flow
# MAGIC * That includes loading Precisely Datasets, joining with the Real Estate Data, selecting the same features, and aggregating into "Proximity Features"

# COMMAND ----------

# DBTITLE 1,Available Datasets
# MAGIC %md
# MAGIC 
# MAGIC As we recall from enriching the 600 Starbucks locations, we have the following datasets available:
# MAGIC 
...
# DBTITLE 1,Load Precisely Datasets
#Address Fabric
US_AF_addresses =  spark.sql(f"select * from gv_usa_address_fabric_001")
...
# DBTITLE 1,Load Real Estate Residential Geofenced Data
#same codes as used for Training Data
#Not using the manual batches as it is not necessary (9+hours vs minutes)

geo_fence_raw1 =  spark.sql("select * from workshop.geofenced_real_estate_residential")
...
geo_fence_raw4 = geo_fence_raw2.drop_duplicates()
...
# MAGIC %md
# MAGIC 
# MAGIC ### Join Real Estate Residential Data with each of the Precisely Datasets
...
# DBTITLE 1,In the same way we did for Starbucks Locations, Select Same "Core" Residential Features 
random.seed(42)
...
# MAGIC %md
# MAGIC ### Residential Feature Engineering
# MAGIC 
# MAGIC 
...
# DBTITLE 1,Census Blocks per Real Estate Location
#define key cols
...
# DBTITLE 1,Average of Demographics per Geofenced Location
...
# DBTITLE 1,Average Consumer Spend per Location
...
# DBTITLE 1,Average of Audience Profiles
...
spark.sql("DROP TABLE IF EXISTS workshop.real_estate_features_residential")
#eng_features.write.format("delta").saveAsTable("workshop.real_estate_features_residential")

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from workshop.real_state_features_residential

# COMMAND ----------

# MAGIC %sql
# MAGIC select count(*) from workshop.real_state_features_residential
